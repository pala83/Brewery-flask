...
class CervezaModel(Model):
    def __init__(self, app):
        super().__init__(app)
        app.config['MYSQL_DATABASE_DB'] = 'brewery'
    ...
    def getAll(self):
            self.__cursor.execute("SELECT * FROM cerveza;")
            registro = self.__cursor.fetchall()
            self.__connection.commit()
            return registro
        
    def getByID(self, id):
        self.__cursor.execute("SELECT * FROM cerveza WHERE id = (%s);", [id])
    ...
    def postEmpleado(self, nombre, correo, nombreFoto="empty"):
        ...
        self.__cursor.execute("INSERT INTO cerveza (nombre, correo, foto) values (%s, %s, %s);", datos)
        return self.__connection.commit()

    def update(self, nombre, correo, nombreFoto, id):
        datos = (nombre, correo, nombreFoto, id)
        self.__cursor.execute("UPDATE cerveza SET nombre=(%s), correo=(%s), foto=(%s) WHERE id=(%s)", datos)
        return self.__connection.commit()

    def delete(self, id):
        self.__cursor.execute("SELECT foto FROM cerveza WHERE id = (%s)", [id])
        ...
        try:
            ...
            pass
        self.__cursor.execute("DELETE FROM cerveza WHERE id = (%s)", [id])
