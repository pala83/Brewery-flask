...
def delete(id):
    sql = "SELECT foto FROM empleado WHERE id = (%s)"
    datos = [id]
...
def update():
    ...
    if _foto.filename != '':
        ...
        sql = f'SELECT foto FROM empleado WHERE id="{id}"'
        cursor.execute(sql)
        conn.commit()
        ...
        sql = f'UPDATE empleado SET foto="{nuevoNombreFoto}" WHERE id="{id}";'
        cursor.execute(sql)
        conn.commit()

    sql = f'UPDATE empleado SET nombre="{_nombre}", correo="{_correo}" WHERE id="{id}"'
    cursor.execute(sql)
    conn.commit()
